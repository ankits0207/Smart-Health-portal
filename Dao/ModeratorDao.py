...
class ModeratorDao(object):
    def create(self, obj_to_add):
        ...
        cursor = con.cursor()
        try:
            cursor.execute("insert into smarthealthdb.moderator values(%s, %s)",
    ...
    def add_qual(self, modobj, qualobj):
        # cursor = MySingletonConnection.MySingletonConnection().get_instance().cursor()
        con = mysql.connector.connect(user='root', password='root', host='127.0.0.1', database='smarthealthdb')
        cursor = con.cursor()
        try:
            cursor.execute("insert into smarthealthdb.moderatorqualification values(%s, %s, %s)",
                           (qualobj.qualificationid, modobj.username, datetime.date.today()))
            con.commit()
            return True
        except Exception as e:
            raise
    ...
    def read(self, username):
        ...
        try:
            query = "select a.*,b.phone from smarthealthdb.user a,smarthealthdb.moderator b where a.username=b.username"
            ...
            if cursor.rowcount > 0:
                ...
                for row in result:
                    if row[0] == username:
                        ...
                        read_phone = row[17]
                        myflag = 1
                        read_obj = Entities.Moderator(read_phone, read_username, read_password,
                                                      read_email1, read_email2, read_firstname, read_lastname,
                                                      read_aboutme,
    ...
    def read_qual(self, username):
        mylist = []
        myflag = 0
        con = mysql.connector.connect(user='root', password='root', host='127.0.0.1', database='smarthealthdb')
        cursor = con.cursor(buffered=True)
        try:
            query = ("select * from smarthealthdb.moderatorqualification")
            cursor.execute(query)
            if cursor.rowcount > 0:
                result = cursor.fetchall()
                for row in result:
    ...
    def update(self, obj_to_update):
        ...
        try:
            cursor.execute(
                "update smarthealthdb.moderator set phone = %s where username = %s",
                (
    ...
    def delete_qual(self, obj):
        myflag = 0
        con = mysql.connector.connect(user='root', password='root', host='127.0.0.1', database='smarthealthdb')
        cursor = con.cursor()
        try:
            #query = "delete from smarthealthdb.moderatorqualification where username = '%s';"
            #cursor.execute(query,(username))
            cursor.execute("delete from smarthealthdb.moderatorqualification where smarthealthdb.moderatorqualification.username = '%s'"%(obj.username))
            con.commit()
            return True
        except Exception as e:
            raise
